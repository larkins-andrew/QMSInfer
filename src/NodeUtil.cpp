...
Node* NodeUtil::findNode(vector<Node> &v, string nodeName) {
	Node *node = NULL;
...
void NodeUtil::nodeCopy(Node *n, Node copyNode) {
...
bool NodeUtil::hasChildrenUKD(Node* node, deque<Node* > UKDNds, int &count) {
	count++;
	if(node == NULL)
		return false;
...
bool NodeUtil::findUKD(Node* node, deque<Node* > UKDNds) {
...
void NodeUtil::getAuxiliaryTable(Node *node) {
...
void NodeUtil::getRegion(Node *node, set<string>* uniqueMATi, map<string, Node> nodeMap) {
...
bool NodeUtil::isNotCareRand(Node node, string checkRand) {
...
void NodeUtil::getNotCareRandomSet(Node node, set<string>& randSet) {
...
void NodeUtil::getNodeMap(Node* node, map<string, Node*>& nodeMap) {
	if(node == NULL) {
		return;
	}
...
int NodeUtil::getHeight(Node* node) {
	if(node == NULL) {
		return 0;
	}
...
void NodeUtil::printTree(Node* node, int indent) {
	if(node == NULL)
		return;
...
string NodeUtil::regularStringInTree(Node* node, int indent) {
	string s = "";
